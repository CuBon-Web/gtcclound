...
class ProductController extends Controller
{
    public function allProduct()
    {
        ...
        $data['content'] = 'none';
        $this->appendProductCategoryTree($data);
    ...
    private function appendProductCategoryTree(array &$data): void
    {
        $data['productCategoryTree'] = $this->buildProductCategoryTree();
    }

    private function buildProductCategoryTree(): array
    {
        $cateCounts = Product::where('status', 1)
        ...
        $typeCounts = Product::where('status', 1)
            ->whereNotNull('type_cate')
            ->selectRaw('type_cate, COUNT(*) as aggregate')
            ->groupBy('type_cate')
            ->pluck('aggregate', 'type_cate')
            ->toArray();

        $typeTwoCounts = Product::where('status', 1)
            ->whereNotNull('type_two')
            ->selectRaw('type_two, COUNT(*) as aggregate')
            ->groupBy('type_two')
            ->pluck('aggregate', 'type_two')
            ->toArray();

        $categories = Category::where('status', 1)->orderBy('id', 'DESC')->get(['id', 'name', 'slug']);
        $types = TypeProduct::where('status', 1)->orderBy('id', 'DESC')->get(['id', 'name', 'slug', 'cate_id']);
        $typeTwos = TypeProductTwo::where('status', 1)->orderBy('id', 'DESC')->get(['id', 'name', 'slug', 'type_id']);

        $typesByCate = [];
        foreach ($types as $type) {
            $typesByCate[$type->cate_id][] = $type;
        }
        $typeTwosByType = [];
        foreach ($typeTwos as $typeTwo) {
            $typeTwosByType[$typeTwo->type_id][] = $typeTwo;
        }
        ...
        foreach ($categories as $category) {
            ...
            $typeItems = [];
            foreach ($typesByCate[$cateId] ?? [] as $type) {
                $typeId = (int) $type->id;
                $children = [];
                foreach ($typeTwosByType[$typeId] ?? [] as $typeTwo) {
                    $typeTwoId = (int) $typeTwo->id;
                    $children[] = [
                        'id' => $typeTwoId,
                        'name' => languageName($typeTwo->name),
                        'slug' => $typeTwo->slug,
                        'count' => (int) ($typeTwoCounts[$typeTwoId] ?? 0),
                    ];
                }
                $typeItems[] = [
                    'id' => $typeId,
            ...
            $tree[] = [
                'id' => $cateId,
                'name' => languageName($category->name),
                'slug' => $category->slug,
                'count' => (int) ($cateCounts[$cateId] ?? 0),
                'children' => $typeItems,
            ];
        }
