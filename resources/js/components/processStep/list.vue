    ...
    <div>
        <div class="row">
            <div class="col-md-12 grid-margin stretch-card">
                <div class="card">
                    <div class="card-body">
                        <h4 class="card-title">Danh sách quy trình làm việc</h4>
                        <vs-button
                            type="gradient"
                            style="float:right;"
                            :disabled="!$hasPermission('processstep.create')"
                            @click="$goIfAllowed('processstep.create', { name: 'addProcessStep' }, 'Bạn không có quyền thêm mới.')"
                        >Thêm mới</vs-button>
                        ...
                        <vs-table stripe :data="list" max-items="15" pagination>
                            ...
                            <template slot-scope="{ data }">
                                <vs-tr :key="indextr" v-for="(tr, indextr) in data">
                                    ...
                                    <vs-td>
                                        <img v-if="tr.icon" :src="tr.icon" alt="icon" style="width: 42px; height: 42px; object-fit: contain;" />
                                        <span v-else>—</span>
                                    </vs-td>
                                    ...
                                    <vs-td>
                                        <router-link :to="{ name: 'editProcessStep', params: { id: tr.id } }">
...
export default {
    ...
    methods: {
        ...mapActions(['listProcessStep', 'loadings', 'deleteProcessStep']),
        loadList() {
            this.loadings(true);
            this.listProcessStep({ keyword: this.keyword })
                .then(response => {
                    this.loadings(false);
        ...
        confirmDestroy(id) {
            ...
            this.$vs.dialog({
                ...
                text: 'Xóa bước này khỏi quy trình',
                accept: this.destroy,
            });
        },
        destroy() {
            this.deleteProcessStep({ id: this.id_item }).then(() => {
                this.loadList();
